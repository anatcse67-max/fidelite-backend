...
function generateEtoiles(points, seuil, couleur, W = 312, H = 144) {
  ...
  function inStar(px, py, cx, cy) {
    ...
    if (dist > outerR + 1) return 0
    if (dist < innerR - 1) return 1
    // 5 branches
    const spikes = 5
    const angle = Math.atan2(dy, dx) + Math.PI / 2
    const sector = (angle / (2 * Math.PI)) * spikes * 2
    const blend = sector - Math.floor(sector)
    const rAtAngle = blend < 0.5
      ? lerp(outerR, innerR, blend * 2)
      : lerp(innerR, outerR, (blend - 0.5) * 2)
    const aa = Math.max(0, Math.min(1, rAtAngle + 0.8 - dist))
    return aa
  }
